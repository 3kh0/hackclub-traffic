<template>
  ...
  <div v-else class="flex flex-col gap-6">
    <div class="grid grid-cols-2 gap-4">
      <StatCard label="Total Events" :value="fmtNum(data?.total ?? 0)" />
    ...
    <div class="flex justify-center items-center gap-2">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <Bars :items="data?.byAction.map(a => ({ label: a.action, value: a.count, color: ACTION_COLORS[a.action] ?? 'rgba(153, 153, 153, 1)' })) || []" />
      </div>
    </div>
...
const bc = computed(() =>
  (data.value?.byAction ?? []).find((a: any) => a.action === 'block')?.count ?? 0
)

const sac = computed(() => {
  const actions = [...new Set((data.value?.timeline ?? []).map((e: any) => e.action))]
...
const countryItems = computed(() =>
  (data.value?.topCountries ?? []).map((c: any) => ({ label: c.countryName, value: c.count }))
)

const pathItems = computed(() =>
  (data.value?.topPaths ?? []).map((p: any) => ({ label: p.path, value: p.count }))
)
</script>
