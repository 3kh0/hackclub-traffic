...
import { createChart, AreaSeries, createTextWatermark, type IChartApi, type ISeriesApi } from 'lightweight-charts'
...
function wave(i: number, phase: number) {
  return 35
    + Math.sin((i + phase) * 0.4) * 18
    + Math.sin((i + phase) * 0.9) * 10
    + Math.sin((i + phase) * 1.7) * 5
}

function generateControls(phase: number) {
  return Array.from({ length: 25 }, (_, i) => wave(i, phase))
}
...
function createChartInstance() {
  ...
  let watermark = createTextWatermark(chart.panes()[0], {
    horzAlign: 'center',
    vertAlign: 'center',
    lines: [
      {
  ...
  let controls = generateControls(timeCounter)
  series.setData(inter(controls, baseTime))
  chart.timeScale().fitContent()

  interval = setInterval(() => {
    if (!series || !chart) return
    timeCounter += 0.6
    controls = generateControls(timeCounter)
    const data = inter(controls, baseTime)
    series.setData(data)
  }, 120)
}
